...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $total_amount = floatval($tuition_fee) + floatval($misc_fee) + floatval($lab_fee) + floatval($other_fee);

    $sql = "UPDATE assessments 
            SET student_id=?, school_year=?, semester=?, tuition_fee=?, miscellaneous_fee=?, laboratory_fee=?, other_fees=?, total_amount=?, due_date=? 
            WHERE assessment_id=?";
    $stmt = $conn->prepare($sql);

    // Correct bind_param: i = integer, s = string, d = double/float
    $stmt->bind_param("issdddddsi", 
        $student_id, 
        $school_year, 
        $semester, 
        $tuition_fee, 
...
<title>Update Assessment - Admin</title>
<style>
/* Reset & Global */
...
/* Container & Section */
.container { max-width:700px; margin:3rem auto; padding:1rem; }
.section {
