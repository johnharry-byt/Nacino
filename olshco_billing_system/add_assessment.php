...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tuition_fee = sanitize($_POST['tuition_fee']);
    $miscellaneous_fee = sanitize($_POST['miscellaneous_fee']);
    $laboratory_fee = sanitize($_POST['laboratory_fee']);
    $other_fees = sanitize($_POST['other_fees']);
    $due_date = sanitize($_POST['due_date']);

    $total_amount = floatval($tuition_fee) + floatval($miscellaneous_fee) + floatval($laboratory_fee) + floatval($other_fees);

    $sql = "INSERT INTO assessments (student_id, school_year, semester, tuition_fee, miscellaneous_fee, laboratory_fee, other_fees, total_amount, due_date) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issddddds", $student_id, $school_year, $semester, $tuition_fee, $miscellaneous_fee, $laboratory_fee, $other_fees, $total_amount, $due_date);
    ...
    <div class="section">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" name="tuition_fee" required>
            </div>

            <div class="form-group">
                <label>Miscellaneous Fee</label>
                <input type="number" step="0.01" name="miscellaneous_fee" required>
            </div>

            <div class="form-group">
                <label>Laboratory Fee</label>
                <input type="number" step="0.01" name="laboratory_fee" required>
            </div>

            <div class="form-group">
                <label>Other Fees</label>
                <input type="number" step="0.01" name="other_fees" required>
            </div>
